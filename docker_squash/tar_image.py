...
class TarImage(Image):
    ...
    def __init__(self, log, tar_path, from_layer=None, tmp_dir=None, tag=None, comment=""):
    ...
    def _load_oci_metadata(self):
        ...
        manifest_path = os.path.join(self.old_image_dir, "blobs", "sha256", manifest_digest.split(':')[1])
        ...
        config_path = os.path.join(self.old_image_dir, "blobs", "sha256", config_digest.split(':')[1])
    ...
    def _load_docker_metadata(self):
        ...
        # Generate image ID from config hash
        config_content = json.dumps(self.old_image_config, sort_keys=True, separators=(',', ':'))
        self.old_image_id = f"sha256:{hashlib.sha256(config_content.encode()).hexdigest()}"
    ...
    def _extract_oci_layers(self):
        ...
        self.log.debug(f"Found {len(self.old_image_layers)} layers in OCI image (including empty layers)")
    ...
    def _extract_docker_layers(self):
        ...
        self.log.debug(f"Found {len(self.old_image_layers)} layers in Docker image (including empty layers)")
    ...
    def _before_squashing(self):
        ...
        if number_of_layers > len(self.old_image_layers):
            raise SquashError(f"Cannot squash {number_of_layers} layers from {len(self.old_image_layers)} total layers")
    ...
    def _squash(self):
        ...
        real_layers_to_squash = [
            layer_id for layer_id in self.layers_to_squash 
            if not layer_id.startswith('<missing-')
        ]
        real_layers_to_move = [
            layer_id for layer_id in self.layers_to_move 
            if not layer_id.startswith('<missing-')
        ]
        
        self.log.debug(f"Filtering for squashing: {len(real_layers_to_squash)} real squash layers, {len(real_layers_to_move)} real move layers")
    ...
    def _move_preserved_layers(self):
        ...
        for layer_id in self.layers_to_move:
            ...
            if self.oci_format:
                ...
                source_version_path = os.path.join(self.old_image_dir, layer_dir, "VERSION")
    ...
    def _generate_new_metadata(self):
        ...
        if "rootfs" in metadata and "diff_ids" in metadata["rootfs"]:
            ...
            metadata["rootfs"]["diff_ids"] = metadata["rootfs"]["diff_ids"][: non_empty_moved_layers]
        ...
        if real_squashed_layers and hasattr(self, 'diff_ids') and self.diff_ids:
            ...
            self.log.debug(f"Added diff_id for squashed layer with {len(real_squashed_layers)} real layers")
        else:
        ...
        if hasattr(self, 'squash_id') and self.squash_id and not self.squash_id.startswith('<missing-'):
        ...
        if not self.oci_format and os.path.exists(os.path.join(self.squashed_dir, "layer.tar")):
    ...
    def _create_squashed_layer_metadata(self):
        ...
        if self.layers_to_move:
            ...
            layer_metadata['parent'] = parent_layer.split(':', 1)[1] if ':' in parent_layer else parent_layer
    ...
    def _create_docker_manifest(self, new_image_id):
        ...
        if self.image_name and self.image_tag:
            ...
            self.log.info(f"Using user-specified tag: {self.image_name}:{self.image_tag}")
        elif self.tag and not (self.image_name and self.image_tag):
            # User provided tag but parsing failed - warn user
            self.log.warning(f"Could not parse tag '{self.tag}', image will have no repository tags")
        else:
            # No user-specified tag - warn about potential consequences
            if self.manifest.get('RepoTags'):
                original_tags = self.manifest['RepoTags']
                self.log.warning(f"No --tag specified. Original image had tags: {original_tags}")
                self.log.warning("Consider using --tag to specify a new tag for the squashed image")
                # Don't reuse original tags to avoid overwriting
                self.log.info("Squashed image will have no repository tags to avoid overwriting original")
            else:
                self.log.info("No --tag specified and original image had no tags. Squashed image will have no repository tags.")
    ...
    def _create_repositories_json(self, image_id, repo_tags):
        ...
        if repositories:
            ...
            self.log.debug(f"Created repositories.json with {len(repositories)} repositories")
        else:
            self.log.debug("No repository tags to write, skipping repositories.json creation")
    ...
    def _create_oci_manifest(self, new_image_id):
        ...
        self.log.warning("OCI output format not fully implemented - creating Docker format")
        self._create_docker_manifest(new_image_id)
    ...
    def _generate_diff_ids(self):
        ...
        for layer_id in self.layers_to_move:
            ...
            if os.path.exists(layer_tar_path):
                ...
                self.log.warning(f"Layer tar not found for diff_id calculation: {layer_tar_path}")

        if self.layers_to_squash and os.path.exists(os.path.join(self.squashed_dir, "layer.tar")):
            sha256 = self._compute_sha256(os.path.join(self.squashed_dir, "layer.tar"))
            diff_ids.append(sha256)

        self.log.debug(f"Generated {len(diff_ids)} diff_ids from {len(self.layers_to_move)} moved layers")
        return diff_ids
    ...
    def load_squashed_image(self):
        ...
        try:
            ...
            self.log.warning("Docker client not available - cannot load image into daemon")
        except Exception as e:
